...
You MUST respond with valid JSON matching this schema:
{
  ...
  "prLevelComments": [
    {
      "path": "OPTIONAL relative/file/path.ts — the one changed file this concerns, omitted when it spans several.",
      "title": "Single-sentence headline describing the finding (ends with a period).",
...
- "prLevelComments" is for findings that are NOT tied to one specific changed line and therefore cannot be an inline comment. Use it — do NOT invent a line number or bury the finding only in the summary — for: the diff contradicting the PR description (a claimed change is missing, or the code does something the description doesn't mention), issues spanning many files, or concerns about the change as a whole. Entries carry NO "line" and the same title/body/type/severity/aiAgentPrompt/confidence fields as inline comments. Omit the field or use [] when there are none.
- "path" on a "prLevelComments" entry is OPTIONAL but strongly preferred, and decides where the finding lands. SET it whenever the finding is about one changed file, even if you reached it by reading several: "the README documents a command the compose change doesn't support" is about \`README.md\`. Such a finding is posted as a review thread on that file — the author can reply to it and resolve it. OMIT it only when the finding genuinely spans multiple files or the change as a whole; those can only be printed in the review body, where nobody can ever dismiss them. When set, "path" must exactly match a changed file — never guess a filename.
...
- Reserve "prLevelComments" for findings that genuinely have no home on a changed line. A path-less entry there is the most expensive thing you can emit: a reader cannot resolve it, reply to it, or collapse it, and it stays in the review body permanently. Prefer, in order: "comments" with a real line > a "prLevelComments" entry WITH a "path" > a path-less entry. Rate "confidence" honestly: a "medium" entry still reaches the reviewer, just without claiming the top of the review.
