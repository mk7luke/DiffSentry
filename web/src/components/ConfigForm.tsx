    ...
    <div className="cfg-list">
      {arr.map((v, i) => (
        <div className="cfg-list-row" key={i}>
          ...
          />
          <button type="button" className="btn btn-ghost cfg-remove" onClick={() => update(arr.filter((_, j) => j !== i))}>
            ✕
          </button>
        </div>
      ))}
      <button type="button" className="btn btn-ghost cfg-add" onClick={() => update([...arr, ""])}>
        + Add
      </button>
    </div>
  );
...
function ObjectArrayField({ value, item, onChange }: { value: unknown; item: JsonSchema; onChange: (v: Obj[] | undefined) => void }) {
  ...
  return (
    <div className="cfg-list">
      ...
      <button type="button" className="btn btn-ghost cfg-add" onClick={() => update([...arr, emptyValueFor(item) as Obj])}>
        + Add
      </button>
    </div>
  );
...
export function ConfigForm({ schema, value, onChange }: { schema: JsonSchema; value: Obj; onChange: (v: Obj) => void }) {
