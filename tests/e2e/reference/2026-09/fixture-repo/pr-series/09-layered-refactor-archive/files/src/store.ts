import { isWithinRetention } from "./retention.js";
...
export class Store {
  ...
  /** All reports across every owner, for the archival job. */
  all(): Report[] {
    return [...this.rows];
  }
  ...
  prune(olderThanMs: number, now: number): number {
    ...
    this.rows = this.rows.filter((r) => isWithinRetention(r.createdAt, now, olderThanMs));
