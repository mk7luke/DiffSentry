...
| `✅ Addressed in commit <sha>` / `commits <sha> to <sha>` | 0 | TWO forms; the singular is the majority — singular 19 September / 3 April, range 15 / 3. **[corrected 2026-09-16]** the earlier figure counted only the range form. **Bot-emitted body text**, after the `<!-- auto-generated reply -->` marker (`2026-09/coderabbit/inline.md:559`, `:823`) — **not** GitHub chrome | Needs per-finding resolution tracking across pushes | A9 |
