...
describe("classifySurface", () => {
  // The six `const body` fixtures below (walkthrough, both status shapes,
  // both "review by kind" shapes, and the nitpick-only review) are copied
  // verbatim from tests/e2e/reference/2026-09/*/raw.json (captured
  // 2026-09-15). The one-liner bodies inlined directly in the other
  // `expect()` calls (review-summary wrapper, inline, chat, other) are
  // synthetic minimal cases, not captures — narrow before trusting them as
  // evidence of real bot shape. The original fixture for the status branch
  // was invented and matched no real bot output, which is how a regex
  // anchored to the wrong shape shipped green — see task-5-report.md fix
  // round 2.
