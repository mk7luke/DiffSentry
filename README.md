...
`/webhooks` + `/webhooks/:id` (admin). Write endpoints: `POST /roles` (admin)
sets/clears a role override; `POST /webhooks/:id/replay` (admin) re-dispatches a
stored delivery. When OAuth is configured every endpoint requires a valid
session (401 JSON otherwise); the queries reuse the same SQL as the legacy
dashboard and no-op gracefully when persistence is disabled.
